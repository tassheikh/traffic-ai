...
export const getTrafficColor = (value) => {
  if (value > 800) return { color: "#ef4444", label: "High",   bg: "bg-red-500/20",    text: "text-red-400" };
  if (value > 500) return { color: "#f59e0b", label: "Medium", bg: "bg-yellow-500/20", text: "text-yellow-400" };
  return                  { color: "#22c55e", label: "Low",    bg: "bg-green-500/20",  text: "text-green-400" };
};

const buildZones = () => CITY_ZONES.map((z) => ({ ...z, traffic: randomTraffic() }));
...
export const TrafficProvider = ({ children }) => {
  ...
  // ── Auto simulation every 4s ──
  useEffect(() => {
    if (!simulationActive) return;
    const interval = setInterval(() => {
      setZones((prev) =>
        prev.map((zone) =>
          Math.random() > 0.5 ? { ...zone, traffic: randomTraffic() } : zone
        )
      );
    }, 4000);
  ...
  // ── Socket listener — fills dashboard history ──
  useEffect(() => {
    socket.on("new_prediction", (data) => {
      const value = data.prediction;
      const congestion = value > 800 ? "High" : value > 500 ? "Medium" : "Low";
      ...
      setPredictionHistory((prev) => {
        const updated = [
          { id: Date.now(), value, time, level: congestion },
          ...prev,
        ];
      ...
      setLiveChartData((prev) => {
        const updated = [...prev, { time, traffic: value, avg: Math.round(value * 0.8) }];
        return updated.slice(-10);
      });

      // Update Delhi zone with real prediction
      setZones((prev) =>
        prev.map((z) => (z.id === 1 ? { ...z, traffic: value } : z))
      );
    });
  ...
  // ── Called from Prediction.jsx after API response ──
